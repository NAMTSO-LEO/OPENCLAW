...
| 模型 | AUC-ROC | CV AUC | 状态 |
|------|----------|--------|------|
| Logistic Regression | 0.489 | 0.459 | ⚠️ |
| Random Forest | 0.489 | 0.490 | ⚠️ |
| Gradient Boosting | 0.496 | 0.496 | ⚠️ |

> Note: AUC接近0.5表明当前模拟数据中响应与特征相关性较弱。真实数据预期AUC > 0.7。

### 7.4 特征重要性

| 排名 | 特征 | 重要性 | 解释 |
|------|------|--------|------|
| 1 | LDH | 0.237 | 乳酸脱氢酶 - 肿瘤活性标志 |
| 2 | PLT | 0.183 | 血小板 - 炎症/造血 |
| 3 | HGB | 0.141 | 血红蛋白 - 营养状态 |
| 4 | WBC | 0.139 | 白细胞 - 免疫状态 |
| 5 | AGE | 0.090 | 年龄 |
| 6 | ALB | 0.086 | 白蛋白 - 营养 |
| 7 | ECOG | 0.057 | 功能状态 |
| 8 | PRIOR_LINES | 0.032 | 既往治疗线数 |

### 7.5 模型改进建议

1. **增加真实数据**: 使用MIMIC/SEER数据提升预测能力
2. **增加影像特征**: 整合CT/MRI特征
3. **增加文本特征**: 使用NLP提取病历信息
4. **时间序列建模**: 使用Transformer处理时序数据
5. **多任务学习**: 同时预测OS和Response
